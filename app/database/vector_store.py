...
class VectorStore:
    """A class for managing vector operations & database interactions."""

    def __init__(self):
        """Initialise vectorstore with settings, OpenAI client & Timescale Vector client"""
    ...
    def get_embeddings(self, text: str) -> List[float]:
        ...
        logging.info(f"Embedding generated in {elapsed_time:.2f} seconds")
        return embedding
    ...
    def upsert(self, df: pd.DataFrame) -> None:
        ...
        Args:
            df: A DataFrame containing embeddings to upsert
        """
        records = df.to_records(index=False)
        self.vec_client.upsert(list(records))
        logging.info(
            f"Upserted {len(records)} embeddings into {self.vector_settings.table_name}"
        )
    ...
    def search(
        ...
        logging.info(f"Vector search completed in {elapsed_time:.2f} seconds")
    ...
    def delete(
        self, ids: List[str] = None, metadata_filter: dict = None, delete_all: bool = False
    ) -> None:
        ...
        if sum(bool(x) for x in [ids, metadata_filter, delete_all]) != 1:
            raise ValueError(
                "Please provide exactly one deletion criteria: ids, metadata_filter, or delete_all"
            )
        ...
        if delete_all:
            ...
            self.vec_client.delete_by_id(ids)
            logging.info(
            ...
            logging.info(
                f"Deleted records with metadata filter {metadata_filter} from {self.vector_settings.table_name}"
            )
